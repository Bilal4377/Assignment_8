...
const dbUrl = "mongodb+srv://admin:Password3@cluster0.yfajg.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
...
//deletes the record of a course
app.post('/deleteCourseById', async (req, res) =>{
    try {
        ...
        if (courses)
        {
            ...
            return res.status(200).json("The course with this id has been deleted");
        }
        else {
        ...
        return res.status(500).json("Failed to delete the specified course");
    }
});
...
app.post('/removeStudentFromClasses', async (req, res) =>{
    try {
        let students = await Student.findOne({studentID: req.body.studentID});

        if (students)
        {
            await Student.deleteOne({studentID: req.body.studentID});
            return res.status(200).json("The student with this id has been deleted");
        }
        else {
            res.status(200).json("No object found");
        }
        ...
        return res.status(500).json("Failed to remove the specified student");
    }
});
